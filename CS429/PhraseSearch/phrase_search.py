...
def tokenize(document):
    ...
    """
    return [word.lower() for word in re.split('[.]|[?]|[\s]|[!]|[,]|[;]|[:]', document) if word != '']
...
def stem(tokens):
    ...
    listReturn = []
    for word in tokens:
        if word == 'did' or word == 'does':
            listReturn.append('do')
        else:
...
def create_positional_index(tokens):
    ...
    positionCounter = 0
    documentID = 0
    for token in tokens: #tokens is a list of lists, token is a list within tokens, term is a value at a position in token 
        for term in token:
            if (term in index):
                if (len(index[term]) == (1 - documentID)): #was documentID in index[term]
                    index[term][documentID].append(positionCounter) 
                    positionCounter = positionCounter + 1
                elif (len(index[term]) != (1 - documentID)): #was opposite of prior comment
                    newlist = []
                    newlist.append(documentID)
                    newlist.append(positionCounter)
                    index[term].append(newlist)
                    positionCounter = positionCounter + 1

            else:
                listDocument = []
                listDocument.append(documentID)
                listDocument.append(positionCounter)
                index[term] = [listDocument] 
                positionCounter = positionCounter + 1
...
def phrase_intersect(list1, list2):
    ...
    while (documentPointer1 <= len(list1)) and (documentPointer2 <= len(list2)):
        if list1[documentPointer1][0] == list2[documentPointer2][0]: 
             
            if ((list2[documentPointer2][positionPointer2] - list1[documentPointer1][positionPointer1]) == 1):
                if len(listIntersection) == 0:
                    listIntersection.append(list2[documentPointer2][0])
                listIntersection.append(list2[documentPointer2][positionPointer2])
                positionPointer1 = positionPointer1 + 1
                positionPointer2 = positionPointer2 + 1
                if (positionPointer1 == len(list1[documentPointer1]) or (positionPointer2 == len(list2[documentPointer2]))):
                    listReturn.append(listIntersection)
                    listIntersection = []
                    positionPointer1 = 1
                    positionPointer2 = 1
                    documentPointer1 = documentPointer1 + 1
                    documentPointer2 = documentPointer2 + 1
                    if (documentPointer1 == len(list1)) or (documentPointer2 == len(list2)):
                        break

            
            elif ((list2[documentPointer2][positionPointer2] - list1[documentPointer1][positionPointer1]) > 1):
                positionPointer1 = positionPointer1 + 1
                if positionPointer1 == len(list1[documentPointer1]):
                    listReturn.append(listIntersection)
                    listIntersection = []
                    documentPointer1 = documentPointer1 + 1
                    documentPointer2 = documentPointer2 + 1
                ...
                positionPointer2 = positionPointer2 + 1
                if positionPointer2 == len(list2[documentPointer2]):
                    listReturn.append(listIntersection)
                    listIntersection = []
                    documentPointer1 = documentPointer1 + 1
                    documentPointer2 = documentPointer2 + 1
                    positionPointer1 = 1
                    positionPointer2 = 1
                    if (positionPointer1 == len(list1)) or (positionPointer2 == len(list2)):
                        break

        elif (list1[documentPointer1][0] > list2[documentPointer2][0]):
            documentPointer2 = documentPointer2 + 1
            listIntersection = []
            if (documentPointer1 == len(list1)) or (documentPointer2 == len(list2)):
                break
    ...
    listReturn = [x for x in listReturn if x != [] ]
...
def search(index, query):
    ...
    i = 1
    if terms[0] not in index.keys():
        return []
    else:
        returnList = index[terms[0]]
    
    while i<len(terms):
        if terms[i] not in index.keys():
            break
        returnList = phrase_intersect(returnList, index[terms[i]])
        i = i + 1
    return [returnList[0] for item in returnList]
...
def find_top_bigrams(terms, n):
    ...
    [('b c', 4), ('a b', 3)]
    """
    #make each term in dictionary have a list, longest list sum the contents of each respective list for the total number of occurences
    index = {}
    i = 0
    for document in terms:
        for term in document:
            if ((i + 1) < len(document)) and (document[i] != document[i + 1]) and ( (document[i] + ' ' + document[i + 1]) not in index):
                term = document[i] + ' ' +document[i + 1]
                index[term] = []
                index[term].append(1)
                i = i + 1
            elif ((i + 1) < len(document)) and (document[i] != document[i + 1]) and ((document[i] + ' ' + document[i + 1]) in index):
                index[(document[i] + ' ' + document[i + 1])].append(1)
                i = i + 1
            elif ((i + 1) >= len(document)):
                i = 0
                break
    ...
    for key in index:
        slist.append( (key , sum(index[key])) )
    
    sortedList = sorted(slist, key=lambda x: x[1])
    ...
    for elt in sortedList:
        listReturn.append(sortedList[-k])
	if (k < n):
            k = k + 1
	else:
	    break
